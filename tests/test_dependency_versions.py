...
def test_lookup_dependency_versions_use_torch_distribution_for_both_aliases(
    monkeypatch,
):
    installed = {
        "numpy": "2.0.0",
    ...
    for backend in ("lookup_transformer", "lookup"):
        versions = dependency_versions(backend)
        assert versions["torch"] == "2.2.2"
        assert "lookup_transformer" not in versions
        assert "lookup" not in versions
...
def test_lookup_torch_dependency_is_written_to_both_json_artifacts(
    tmp_path, baseline_config_path, monkeypatch
):
    config = load_config(baseline_config_path)
    config["experiment"]["name"] = "lookup_artifact_contract"
    ...
    for payload in (metrics, experiment):
